...
namespace game
{
	...
	WEAK symbol<void(gentity_s* ent, float* position, float* orientation)> TeleportPlayer{0x50D840};

	WEAK symbol<void(pmove_t*, trace_t*, const float*,
		const float*, const Bounds*, int, int)> PM_playerTrace{0x421F00};
	WEAK symbol<void(const pmove_t*, trace_t*, const float*,
		const float*, const Bounds*, int, int)> PM_trace{0x41CEB0};
	WEAK symbol<EffectiveStance(const playerState_s*)> PM_GetEffectiveStance{0x41D1E0};
	...
	WEAK symbol<bool(const unsigned int* const, Weapon, bool)> BG_WeaponBulletFire_ShouldPenetrate{0x431250};
	WEAK symbol<bool(const unsigned int* const, Weapon, bool)> BG_WeaponBulletFire_ShouldSpread{0x438F00};
	WEAK symbol<void(gentity_s*, const gentity_s*, gentity_s*,
		int, int, Weapon, bool, const float*, hitLocation_t, int)> player_die{0x503460};
	...
	WEAK symbol<void(int localClientNum, const char* text)> Cbuf_InsertText{0x545770};
	WEAK symbol<void(int localClientNum, int a2, const char* message, int a4, int a5, int a6)> Client_Print{0x487B00};
	...
	WEAK symbol<void(client_s *drop, const char *reason, int notifyOthers)> SV_DropClientInternal{0x570980};
	WEAK symbol<void(client_s* drop, const char* reason)> SV_DelayDropClient{0x570BB0};
	...
	WEAK symbol<void(const char* name, const char* value)> Dvar_SetStringByName{0x5BF660};

	WEAK symbol<const char*(const char* s, const char* key)> Info_ValueForKey{0x5C2DB0};
	...
	WEAK symbol<void(char* s, const char* key, const char* value)> Info_SetValueForKey{0x5C3170};
	...
	WEAK symbol<bool(netsrc_t, netadr_s dest, const char* message)> NET_OutOfBandPrint{0x55C830};
	WEAK symbol<void(gentity_s* ent, int a2, int which_chat, const char* message)> Cmd_Say_f{0x502B60};
	WEAK symbol<void()> Com_Quit_f{0x5556B0};
	WEAK symbol<void(int index, char* buffer, int bufferSize)> SV_GetUserinfo{0x573E00};
	WEAK symbol<bool(const char* s)> Info_Validate{0x5C3140};
	WEAK symbol<void(const char*, const StringTable**)> StringTable_GetAsset{0x5B87D0};
	WEAK symbol<int(const StringTable*, int, const char*)> StringTable_LookupRowNumForValue{0x5B8610};
	WEAK symbol<const char*(const StringTable*, int, int)> StringTable_GetColumnValueForRow{0x5B86A0};
