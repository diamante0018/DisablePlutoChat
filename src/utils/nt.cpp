...
namespace utils::nt
{
  ...
  PIMAGE_NT_HEADERS library::get_nt_headers() const
  {
    ...
    return reinterpret_cast<PIMAGE_NT_HEADERS>(
        this->get_ptr() + this->get_dos_header()->e_lfanew);
  }
  ...
  void library::unprotect() const
  {
    ...
    DWORD protection;
    VirtualProtect(this->get_ptr(),
                   this->get_optional_header()->SizeOfImage,
                                ...
                                const std::string& proc_name) const
  {
    ...
    {
      if (!_stricmp(reinterpret_cast<char*>(this->get_ptr() +
  ...
  void raise_hard_exception()
  {
    ...
    ntdll.invoke_pascal<void>(
        "NtRaiseHardError", 0xC000007B, 0, nullptr, nullptr, 6, &data);
  }
  ...
  std::string load_resource(const int id)
  {
    ...
    return std::string(LPSTR(LockResource(handle)),
                       SizeofResource(nullptr, res));
  }
  ...
  void relaunch_self()
  {
    ...
    CreateProcessA(self.get_path().data(),
                   command_line,
                   nullptr,
                   nullptr,
                   false,
